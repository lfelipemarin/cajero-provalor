...
if (!(isset($_POST['updatepass']) )) {
    session_start();
    if ($_SESSION["name"] || $_SESSION["name_admin"]) {
        ?>
    ...
    if ($_POST['password'] == $_POST['conpassword']) {
        echo "<script type='text/javascript' charset='UTF-8'>alert('" . $usr->updatePass() . "'); window.location='userLogout.php'</script>";
    } else {
        echo "<script type='text/javascript' charset='UTF-8'>alert('Las Contrase\u00f1as no Coinciden');</script>";
        header("Refresh:0");
    }
}
?>